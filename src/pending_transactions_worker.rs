...
pub async fn run(rpc_url: url::Url,
    ...
    loop {
        ...
        while let Some(pair) = iter.next_item().await {
            let hash = web3::types::H256::from_str(pair.0.as_str()).expect("Unable to parse tx hash");
        ...
        for item in transactions_to_remove {
            let res: redis::RedisResult<()> = redis.connection.hdel(async_redis_wrapper::PENDING_TRANSACTIONS, item.to_string()).await;
