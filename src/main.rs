...
async fn main() {
    ...
    let near_client = near_client::methods::NearClient::init(
        near_settings.private_key.unwrap(),
        near_settings.rpc_url.unwrap(),
        near_settings.contract_address.unwrap(),
    );

    let eth_settings = settings.near_settings.clone();
    let eth_client = eth_client::methods::EthClient::init(
        eth_settings.private_key.unwrap(),
        eth_settings.rpc_url.unwrap(),
        eth_settings.contract_address.unwrap(),
    );
