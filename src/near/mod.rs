    ...
    while let Some(streamer_message) = stream.recv().await {
        ...
        for shard in streamer_message.shards {
            for outcome in shard.receipt_execution_outcomes {
                let watching_list = &[
                    AccountId::from_str("br_misha.testnet").unwrap(),
                    AccountId::from_str("weth_beta.nearlend.testnet").unwrap(),
                ];

                if watching_list.contains(&outcome.receipt.receiver_id) {
                    println!(
                        "predecessor_id: {:?}, receiver_id: {:?}",
                        outcome.receipt.predecessor_id, outcome.receipt.receiver_id
                    );
